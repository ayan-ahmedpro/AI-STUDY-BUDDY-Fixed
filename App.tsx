...
export default function App() {
  ...
  const [quizViewMode, setQuizViewMode] = useState<'step' | 'all'>('step');
              ...
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'quiz' && (
                    <motion.div 
                        ...
                        exit={{ opacity: 0, x: -20 }}
                    >
                         <div id="quiz" className="bg-slate-900 rounded-[3rem] sm:rounded-[4rem] p-6 sm:p-12 text-white shadow-2xl relative overflow-hidden ring-4 ring-white/10 shadow-slate-900/40">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-purple-500/20 rounded-full blur-[80px]" />
                            
                            {/* Quiz Header & Top Navigation Bar */}
                            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 border-b border-white/10 pb-6">
                              <div className="flex flex-wrap items-center gap-4">
                                  <button
                                      ...
                                      className="px-4 py-2.5 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs transition-all flex items-center gap-2 border border-white/15 cursor-pointer group shrink-0"
                                  >
                                  ...
                                  <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                          <h3 className="text-2xl sm:text-3xl font-black tracking-tight">Concept Mastery Quiz</h3>
                                          <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px]">
                                              {(result?.quiz || []).length} AI Assessment Questions
                                          </p>
                                      </div>
                                  </div>
                              </div>

                              {/* View Mode Toggle & Main Validate Button */}
                              <div className="flex items-center gap-3 self-end md:self-auto">
                                  <div className="flex items-center bg-white/5 p-1 rounded-2xl border border-white/10">
                                      <button
                                          type="button"
                                          onClick={() => setQuizViewMode('step')}
                                  ...
                                  {!showQuizResults && (
                                      <button 
                                          ...
                                          className={`px-6 py-2.5 rounded-2xl font-black transition-all shadow-xl text-xs uppercase tracking-wider cursor-pointer ${
                                              Object.keys(selectedAnswers).length > 0
                                                  ? 'bg-purple-500 hover:bg-purple-400 text-white shadow-purple-500/25 hover:scale-105 active:scale-95'
                                                  : 'bg-white/10 text-slate-500 cursor-not-allowed'
                                          }`}
                                      >
                                          Validate Answers ({Object.keys(selectedAnswers).length}/{(result?.quiz || []).length})
                                      </button>
                                  )}
                              </div>
                            </div>

                            {/* Quiz Results Score Summary Banner */}
                            {showQuizResults && (
                              <motion.div 
                                initial={{ opacity: 0, y: -10 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="relative z-10 mb-8 p-6 rounded-3xl bg-gradient-to-r from-purple-900/60 to-indigo-900/60 border border-purple-500/30 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl"
                              >
                                <div className="flex items-center gap-4">
                                  <div className="w-14 h-14 rounded-2xl bg-purple-500/20 border border-purple-400/30 flex items-center justify-center shrink-0 text-purple-300">
                                  ...
                                  <div>
                                    ...
                                    <h4 className="text-2xl font-black text-white">
                                      Score: {(result?.quiz || []).filter((q, i) => selectedAnswers[i] === q.correctAnswer).length} / {(result?.quiz || []).length} Correct
                                    </h4>
                                    <p className="text-xs text-purple-200 mt-0.5">
                                      Great active recall practice! Review correct answers highlighted below.
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                  <button
                                    ...
                                    className="px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl font-bold text-xs transition-all flex items-center gap-2 cursor-pointer border border-white/15"
                                  >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => setActiveTab('overview')}
                                    className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-xl font-black text-xs transition-all flex items-center gap-2 cursor-pointer shadow-lg shadow-purple-600/30"
                                  >
                            ...
                            {/* Question Jump Pills Stepper Navigation */}
                            {(result?.quiz || []).length > 0 && (
                              <div className="relative z-10 mb-8 flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
                                {(result?.quiz || []).map((q, idx) => {
                                  const isCurrent = currentQuizQuestionIndex === idx && quizViewMode === 'step';
                                  const isAnswered = selectedAnswers[idx] !== undefined;
                                  const isCorrect = showQuizResults && selectedAnswers[idx] === q.correctAnswer;
                                  const isWrong = showQuizResults && isAnswered && !isCorrect;
                                  ...
                                  return (
                                    <button
                                      ...
                                      type="button"
                                      onClick={() => {
                                        setCurrentQuizQuestionIndex(idx);
                                        setQuizViewMode('step');
                                      }}
                                      className={`px-3.5 py-1.5 rounded-xl font-black text-xs transition-all flex items-center gap-1.5 shrink-0 cursor-pointer border ${
                                        isCurrent
                                          ...
                                          : isAnswered
                                          ? 'bg-purple-950 text-purple-200 border-purple-700/50'
                                          : 'bg-white/5 text-slate-400 border-white/10 hover:bg-white/10 hover:text-white'
                                      }`}
                                    >
                                      <span>Q{idx + 1}</span>
                                      {isAnswered && !showQuizResults && <div className="w-1.5 h-1.5 rounded-full bg-purple-400" />}
                                      {showQuizResults && isCorrect && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />}
                                    </button>
                                  );
                                })}
                              </div>
                            ...
                            <div className="space-y-12 relative z-10">
                              {quizViewMode === 'step' ? (
                                /* Step-by-Step Single Question View */
                                (() => {
                                  const quizList = result?.quiz || [];
                                  if (quizList.length === 0) {
                                    return (
                                      <div className="text-center py-12 text-slate-400">
                                        <p>No quiz questions available for this study material yet.</p>
                                      </div>
                                    );
                                  }

                                  const qIdx = Math.min(currentQuizQuestionIndex, quizList.length - 1);
                                  const q = quizList[qIdx];
                                  if (!q) return null;

                                  return (
                                    <div className="space-y-8 bg-white/5 border border-white/10 rounded-[2.5rem] p-6 sm:p-8 backdrop-blur-sm">
                                      <div className="flex items-center justify-between border-b border-white/10 pb-4">
                                        <span className="text-purple-400 font-black text-sm uppercase tracking-widest">
                                          Question {qIdx + 1} of {quizList.length}
                                        </span>
                                      ...
                                      <div className="flex items-start gap-4">
                                        <span className="text-purple-400 font-black text-3xl opacity-60 shrink-0">Q{qIdx + 1}</span>
                                        <p className="font-bold text-xl sm:text-2xl leading-snug text-slate-100">{q?.question}</p>
                                      </div>
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {(q?.options || []).map((opt, oIdx) => {
                                          ...
                                          return (
                                            <button
                                              key={`quiz-opt-step-${qIdx}-${oIdx}`}
                                              onClick={() => handleAnswerSelect(qIdx, opt)}
                                              disabled={showQuizResults}
                                              className={cn(
                                                "p-5 sm:p-6 rounded-[2rem] border-2 text-left transition-all font-bold text-base sm:text-lg relative overflow-hidden group/opt cursor-pointer",
                                                isSelected 
                                                  ? "border-purple-500 bg-white text-slate-950 shadow-xl" 
                                                  : "border-white/10 hover:border-purple-500/50 bg-white/5 text-slate-300",
                                                showCorrect && "border-emerald-500 !bg-emerald-500 !text-white shadow-emerald-500/20",
                                                showWrong && "border-rose-500 !bg-rose-500 !text-white shadow-rose-500/20"
                                              )}
                                            >
                                              <div className="flex items-center justify-between pointer-events-none">
                                                <span>{opt}</span>
                                                {showCorrect && <CheckCircle2 className="w-6 h-6 text-white shrink-0" />}
                                              </div>
                                            </button>
                                          );
                                        })}
                                      ...
                                      <div className="flex items-center justify-between pt-6 border-t border-white/10 gap-4">
                                        ...
                                        <div className="text-xs font-bold text-slate-400">
                                          {qIdx + 1} / {quizList.length}
                                        </div>

                                        {qIdx < quizList.length - 1 ? (
                                          <button
                                            type="button"
                                            onClick={() => setCurrentQuizQuestionIndex(prev => Math.min(quizList.length - 1, prev + 1))}
                                            className="px-5 py-3 rounded-2xl bg-purple-600 hover:bg-purple-500 text-white font-black text-xs transition-all flex items-center gap-2 cursor-pointer shadow-lg shadow-purple-600/30"
                                          >
                                          ...
                                          <button
                                            ...
                                            onClick={handleAnswerValidation}
                                            disabled={Object.keys(selectedAnswers).length === 0}
                                            className="px-6 py-3 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs transition-all flex items-center gap-2 cursor-pointer shadow-lg shadow-emerald-500/20 uppercase tracking-wider"
                                          >
                                          ...
                                          <button
                                            type="button"
                                            onClick={() => setActiveTab('overview')}
                                            className="px-5 py-3 rounded-2xl bg-purple-600 hover:bg-purple-500 text-white font-black text-xs transition-all flex items-center gap-2 cursor-pointer"
                                          >
                                            <ArrowLeft className="w-4 h-4" />
                                            <span>Back to Overview</span>
                                          </button>
                                ...
                                (result?.quiz || []).map((q, qIdx) => (
                                    <div key={`quiz-q-all-${qIdx}`} className="space-y-6 bg-white/5 border border-white/10 rounded-[2.5rem] p-6 sm:p-8">
                                    <div className="flex items-start gap-4">
                                        <span className="text-purple-400 font-black text-2xl opacity-50">Q{qIdx + 1}</span>
                                        <p className="font-bold text-xl sm:text-2xl leading-tight text-slate-100">{q?.question}</p>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {(q?.options || []).map((opt, oIdx) => {
                                        const isSelected = selectedAnswers[qIdx] === opt;
                                        ...
                                        return (
                                            <button
                                            key={`quiz-opt-all-${qIdx}-${oIdx}`}
                                            onClick={() => handleAnswerSelect(qIdx, opt)}
                                            disabled={showQuizResults}
                                            className={cn(
                                                "p-5 rounded-[2rem] border-2 text-left transition-all font-bold text-base sm:text-lg relative overflow-hidden group/opt cursor-pointer",
                                                isSelected 
                            ...
                            <div className="relative z-10 mt-12 pt-6 border-t border-white/10 flex items-center justify-between">
                              <button
                                type="button"
                                onClick={() => setActiveTab('overview')}
                                className="px-5 py-3 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs transition-all flex items-center gap-2 border border-white/15 cursor-pointer"
                              >
                                <ArrowLeft className="w-4 h-4" />
                              ...
                              <p className="text-xs text-slate-400 font-medium">
                                Practice makes permanent • Grounded AI Assessment
                              </p>
                            </div>
                        </div>
                    </motion.div>
